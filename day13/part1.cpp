...
typedef std::array<std::array<double, 2>, 2> matrix2x2;

double det(matrix2x2 mat)
{
...
int main()
{
    ...
    std::array<double, 2> steps;
    const auto costs = std::array<int32_t, 2>{3, 1};
    ...
    while (getline(f, line))
    {
        ...
        getline(ss, tok, ' ');
        target[0] = std::stod(tok);
        getline(ss, tok, ' ');
        target[1] = std::stod(tok);
